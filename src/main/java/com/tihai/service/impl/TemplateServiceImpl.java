/*
 * Copyright 2005-2013 setting.net. All rights reserved.
 * Support: http://www.setting.net
 * License: http://www.setting.net/license
 */
...
import com.tihai.common.Template.Type;
...
public class TemplateServiceImpl implements TemplateService, ServletContextAware {
	...
	public List<Template> getAll() {
		try {
			...
			List<Element> elements = document.selectNodes("/setting/template");
	...
	public List<Template> getList(Type type) {
		if (type != null) {
			try {
				...
				Document document = new SAXReader().read(settingXmlFile);
				List<Template> templates = new ArrayList<Template>();
				List<Element> elements = document.selectNodes("/setting/template[@type='" + type + "']");
				for (Element element : elements) {
					Template template = getTemplate(element);
					templates.add(template);
				}
