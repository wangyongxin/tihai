...
import com.tihai.common.Template;
import com.tihai.dao.ArticleDao;
import com.tihai.dao.BrandDao;
import com.tihai.dao.ProductDao;
import com.tihai.dao.PromotionDao;
import com.tihai.entity.Article;
import com.tihai.entity.Brand;
import com.tihai.entity.Product;
import com.tihai.entity.Promotion;
import com.tihai.service.StaticService;
import com.tihai.service.TemplateService;
	...
	public int buildSitemap() {
		...
		for (int step = 0, index = 0, first = 0, count = SITEMAP_MAX_SIZE;;) {
			try {
				...
				if (step == 0) {
					List<Article> articles = articleDao.findList(first, count);
...
//						articleDao.flush();
					...
					List<Product> products = productDao.findList(first, count);
...
//						productDao.flush();
					...
					List<Brand> brands = brandDao.findList(first, count);
...
//						brandDao.flush();
					...
					List<Promotion> promotions = promotionDao.findList(first, count);
...
//					promotionDao.flush();
	...
	public int buildAll() {
		int buildCount = 0;
		for (int i = 0; i < articleDao.countAll(); i += 20) {
			List<Article> articles = articleDao.findList(i, 20);
			for (Article article : articles) {
				buildCount += build(article);
			}
//			articleDao.clear();
		}
		for (int i = 0; i < productDao.countAll(); i += 20) {
			List<Product> products = productDao.findList(i, 20);
